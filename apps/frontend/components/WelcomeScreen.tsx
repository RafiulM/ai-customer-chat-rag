...
import {
  ApiService,
  RagStoresListResponse,
  RagStoreDetailResponse,
} from "../src/services/apiService";

interface WelcomeScreenProps {
  onUpload: () => Promise<void>;
  onResumeSession: (ragStoreName: string, displayName: string) => Promise<void>;
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({
  ...
  // Fetch previous RAG stores on mount, but only if user is signed in
  useEffect(() => {
    ...
    const fetchPreviousSessions = async () => {
      ...
      try {
        const response =
          (await apiService.getRagStores()) as RagStoresListResponse;
        if (response.ragStores) {
          // Fetch chat count for each store
          const sessionsWithCounts = await Promise.all(
            response.ragStores.map(async (store) => {
              try {
                const detailResponse = (await apiService.getRagStore(
                  store.name
                ...
                return {
                  ...
                  chatCount: detailResponse?.chatCount || 0,
                };
              } catch (error) {
                console.warn(
                  `Failed to get chat count for ${store.name}:`,
                  error
                );
                return {
                  name: store.name,
          ...
          setPreviousSessions(sessionsWithCounts);
        }
      } catch (error) {
        console.warn("Failed to fetch previous sessions:", error);
  ...
  }, [apiService, isLoaded, isSignedIn]);

  const handleResume = async (ragStoreName: string, displayName: string) => {
    setResumingSession(ragStoreName);
    try {
      await onResumeSession(ragStoreName, displayName);
    } catch (error) {
  ...
  const handleSelectSample = async (
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("Failed to fetch")) {
        alert(
          `Could not fetch the sample document. Please try uploading a local file instead.`
        );
      }
    } finally {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full overflow-y-auto p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-3xl text-center">
        ...
        <p className="text-gem-offwhite/70 mb-8">
          Powered by{" "}
          <strong className="font-semibold text-gem-offwhite">
            FileSearch
          </strong>
          {isAllowedUser
            ? ". Upload a manual or select example to see RAG in action."
            : ". You are connected to the default document."}
        </p>

        {apiKeyError && (
          <div className="w-full max-w-xl mx-auto mb-8">
            <div className="w-full bg-red-900/20 border border-red-500/50 rounded-lg py-3 px-5 text-center text-red-300">
              <p className="font-semibold mb-1">⚠ API Key Error</p>
          ...
          <>
            ...
            >
              <div className="flex flex-col items-center justify-center">
                <UploadCloudIcon />
                <p className="mt-4 text-lg text-gem-offwhite/80">
                  Drag & drop your PDF, .txt, or .md file here.
                </p>
                <input
                  id="file-upload"
                  type="file"
              ...
              <div className="w-full max-w-xl mx-auto mb-6 text-left">
                ...
                <ul className="max-h-36 overflow-y-auto space-y-1 pr-2">
                  {files.map((file, index) => (
                    <li
                      key={`${file.name}-${index}`}
                      className="text-sm bg-gem-mist/50 p-2 rounded-md flex justify-between items-center group"
                    >
                      <span className="truncate" title={file.name}>
                        {file.name}
                      </span>
                      <div className="flex items-center flex-shrink-0">
                        <span className="text-xs text-gem-offwhite/50 ml-2">
            ...
            <div className="w-full max-w-xl mx-auto">
              {files.length > 0 && (
                <button
                  ...
                  title={
                    apiKeyError
                      ? "Please fix the API key error first"
                      : "Start chat session with the selected files"
                  }
                >
                  Upload and Chat
                </button>
        ...
        {/* Previous Sessions */}
        {isAllowedUser && previousSessions.length > 0 && (
          <>
            ...
            <div className="w-full max-w-xl mx-auto mb-6">
              ...
              {loadingSessions ? (
                <div className="flex items-center justify-center py-8">
                  <Spinner />
                  <span className="ml-3 text-gem-offwhite/70">
                    Loading sessions...
                  </span>
                </div>
              ) : (
                <div className="space-y-2">
                  {previousSessions.map((session) => (
                    <button
                      key={session.name}
                      onClick={() =>
                        handleResume(session.name, session.displayName)
                      }
                      disabled={!!resumingSession}
                      className="w-full bg-gem-slate p-4 rounded-lg border border-gem-mist/30 hover:border-gem-blue/50 hover:bg-gem-mist/10 transition-all text-left disabled:opacity-50 disabled:cursor-not-allowed"
                      title={`Resume chat with ${session.displayName}`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gem-offwhite truncate">
                            {session.displayName}
                          </p>
                        ...
                        {resumingSession === session.name ? (
                          <Spinner />
                        ) : (
                          <span className="ml-4 text-gem-blue">Resume →</span>
                        )}
                      </div>
                    </button>
                  ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
              {sampleDocuments.map((doc) => (
                ...
                >
                  ...
                  <div>
                    <p className="font-semibold text-gem-offwhite">
                      {doc.name}
                    </p>
                    <p className="text-sm text-gem-offwhite/60">
                      {doc.details}
