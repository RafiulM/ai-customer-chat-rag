...
const validateRagStore = [
  body("name")
    ...
    .matches(/^[a-zA-Z0-9_\/-]+$/)
    .withMessage(
      "Name can only contain letters, numbers, underscores, hyphens, and forward slashes"
    ),
  body("displayName")
    .trim()
    .isLength({ min: 1, max: 255 })
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const existingStore = await db
        .select()
        .from(ragStores)
        .where(and(eq(ragStores.name, name), eq(ragStores.userId, userId)))
        .limit(1);

      if (existingStore.length > 0) {
        return res
          .status(409)
          .json({ error: "RAG store with this name already exists" });
      }
...
router.get("/:name", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const ragStore = await db
      .select()
      .from(ragStores)
      .where(and(eq(ragStores.name, storeName), eq(ragStores.userId, userId)))
      .limit(1);
    ...
    const chatCount = await db
      .select()
      .from(chats)
      .where(and(eq(chats.userId, userId), eq(chats.ragStoreName, storeName)))
...
router.put("/:name", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!updatedStore) {
      return res
        .status(404)
        .json({ error: "RAG store not found or access denied" });
    }
...
router.delete("/:name", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      message: "RAG store and associated chats deleted successfully",
    });
  } catch (error) {
